...
with st.sidebar:
    ...
    with st.expander("⚙ Column Mapping", expanded=False):
        ...
        zsd_file.seek(0) if hasattr(zsd_file, "seek") else None
        raw_zsd, load_err = load_zsd(zsd_bytes)

        all_types = get_transaction_types(raw_zsd) if raw_zsd is not None else []
        ...
        credit_types = st.multiselect(
            "Credit Note types",
            all_types,
            default=[t for t in all_types if "CREDIT" in t.upper() or t.upper() in ("RE", "REN")],
            key="cred_types",
        )
        challan_types = st.multiselect(
            "Delivery Challan types",
            all_types,
            default=[t for t in all_types if "CHALLAN" in t.upper() or "DELIVERY" in t.upper()],
            key="ch_types",
        )
    ...
    plant_sel = st.selectbox("Plant", ["All Plants"])  # updated after data loads
...
if not invoice_types:
    st.warning("No Invoice types selected. Please configure column mapping in the sidebar.")
    st.stop()

with st.spinner("Processing data..."):
    dfs = build_all_dataframes(
        zsd_bytes, nysd_bytes, tp_bytes,
        invoice_types, credit_types, challan_types,
    )
...
# Populate Plant dropdown now that data is loaded
all_plants = ["All Plants"]
if orders is not None and "Plant" in orders.columns:
    all_plants += sorted(orders["Plant"].dropna().unique().tolist())

# Re-render plant selector with real values using session state
if "plant_sel" not in st.session_state:
    st.session_state["plant_sel"] = "All Plants"
plant_sel = st.sidebar.selectbox(
    "Plant", all_plants, key="plant_sel_real"
)

# Apply filters to every DataFrame
def flt(df):
...
with tab1:
    # SELECTOR BAR
    st.markdown(selector_bar(plant_sel, zone_sel), unsafe_allow_html=True)
    ...
    inv_cards = [
        ...
        ("STOCK COVER (DAYS)",       f"{inv_kpis['stock_cover']:.1f}",             "days of forward cover",    "#E67E22", "#FFFFFF"),
        ("CASES LOADED / MANHOUR",   f"{inv_kpis['cases_per_manhour']:.1f}",       f"based on {fixed_manpower} manpower", "#E67E22", "#FFFFFF"),
    ...
    health_cards = [
        ...
        ("30-45 DAYS TO EXPIRY",    fmt_indian(exp_kpis["near_45_cases"]),    "cases expiring in 31-45 days",   "#F39C12", "#F39C12"),
        ("45-60 DAYS TO EXPIRY",    fmt_indian(exp_kpis["near_60_cases"]),    "cases expiring in 46-60 days",   "#F1C40F", "#F1C40F"),
    ]
...
with tab2:
    ...
    st.dataframe(
        error_df.style.applymap(color_severity, subset=["Severity"]),
    ...
    with st.expander("Inventory Accuracy by Plant"):
        if f_inv_acc is not None and not f_inv_acc.empty:
            def acc_style(val):
                ...
                return ""
            styled = f_inv_acc.style.applymap(acc_style, subset=["Accuracy_%"])
...
with tab3:
    if transport is None or transport.empty:
        ...
        tp = f_transport if f_transport is not None else transport
        ...
        if "Dest_State" in tp.columns and "Billing_Qty" in tp.columns:
            ...
            state_grp = (
                tp.groupby("Dest_State").agg(
                    Cases=("Billing_Qty", "sum"),
                    Shipments=("Billing_Qty", "count"),
